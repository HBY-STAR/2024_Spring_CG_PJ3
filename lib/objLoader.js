...
OBJDoc.prototype.getDrawingInfoPhong = function() {
  // Create an arrays for vertex coordinates, normals, colors, and indices
  var numIndices = 0;
  for(var i = 0; i < this.objects.length; i++){
    numIndices += this.objects[i].numIndices;
  ...
  for(var i = 0; i < this.objects.length; i++){
    ...
    for(var j = 0; j < object.faces.length; j++){
      ...
      var faceNormal = new Normal(0.0, 0.0, 0.0);
      var faceNormalArray = [];
      var faceWeight= [];
      ...
      for(var k = 0; k < face.vIndices.length; k++){
        ...
        faceNormalArray = [];
        for(var l = 0; l < object.faces.length; l++){
          for(var m = 0; m < object.faces[l].vIndices.length; m++){
            if(object.faces[l].vIndices[m] === vIdx){
              faceNormalArray.push(object.faces[l].normal);
              faceWeight.push(1);
              break;
            }
          }
        ...
        for(var l = 0; l < faceNormalArray.length; l++){
            faceNormal.x += faceNormalArray[l].x*faceWeight[l];
            faceNormal.y += faceNormalArray[l].y*faceWeight[l];
            faceNormal.z += faceNormalArray[l].z*faceWeight[l];
        }
        let vecNormal = new Vector3([faceNormal.x, faceNormal.y, faceNormal.z]);
        vecNormal.normalize();
        faceNormal.x = vecNormal.elements[0];
        faceNormal.y = vecNormal.elements[1];
        faceNormal.z = vecNormal.elements[2];
    ...
    // 遍历所有的面，若面中有顶点与面的法向量方向相反，则将该顶点的法向量取反
    // for(var j = 0; j < object.faces.length; j++){
    //   var face = object.faces[j];
    //   for(var k = 0; k < face.vIndices.length; k++){
    //     var vIdx = face.vIndices[k];
    //     var dot = normals[vIdx*3] * face.normal.x + normals[vIdx*3+1] * face.normal.y + normals[vIdx*3+2] * face.normal.z;
    //     if(dot < 0){
    //       normals[vIdx*3] = -normals[vIdx*3];
    //       normals[vIdx*3+1] = -normals[vIdx*3+1];
    //       normals[vIdx*3+2] = -normals[vIdx*3+2];
    //     }
    //   }
    // }
  }
